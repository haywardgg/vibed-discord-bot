...
| **Auto-reconnect & self-heal** | Detects unexpected voice disconnects and retries with exponential backoff (3s → 6s → 12s). Every 5 seconds it also compares Discord's own view of the bot's voice channel with the local client and rebuilds the voice session when the two disagree (at most once a minute), so a stale connection — bot missing from the channel, or connected but silent — recovers without a service restart. If listeners are present and playback still will not start, it escalates to a fresh voice connection (60s, backing off to 10 minutes). Intentional AFK leaves (empty channel) do not trigger reconnection — the bot waits for someone to join instead. |
...
| Bot disconnected and won't rejoin | If the bot was AFK-disconnected (empty channel), it will rejoin automatically when someone enters the voice channel. A stale voice session (bot missing from the channel, or connected with no audio) is now rebuilt automatically within a minute by the 5-second self-heal check. If it is still missing after that, restart the bot service. |
