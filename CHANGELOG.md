...
- **Timeouts on the recovery path actually fire.** `asyncio.wait_for()` alone was not enough: discord.py's voice teardown swallows cancellation while the network is down, so `wait_for` waited for it anyway (observed: an 11-minute stall). The teardown and the channel-history purge are now bounded so a hang is abandoned and logged instead of blocking the caller.
